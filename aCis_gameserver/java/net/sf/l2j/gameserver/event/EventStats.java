...
{
	protected HashMap<Integer, int[]> tempTable;
	private HashMap<Integer, HashMap<Integer, StatModell>> stats;
	private HashMap<Integer, ShowBoard> htmls;
	private HashMap<Integer, int[]> statSums;
	...
	protected void applyChanges()
	{
		...
		for (Player player : EventManager.getInstance().getCurrentEvent().getPlayerList())
		{
			...
			if (!stats.containsKey(playerId))
				stats.put(playerId, new HashMap<Integer, StatModell>());
		...
		try
		{
			...
			while (rset.next())
			{
				if (!stats.containsKey(rset.getInt("player")))
					stats.put(rset.getInt("player"), new HashMap<Integer, StatModell>());
